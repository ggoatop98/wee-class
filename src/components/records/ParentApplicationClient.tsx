...
export default function ParentApplicationClient({ studentId, studentName }: ParentApplicationClientProps) {
    const { user } = useAuth();
    ...
    const formTitle = "학부모 신청서";
    const collectionName = "parentApplications";
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            if (!snapshot.empty) {
                const data = snapshot.docs[0].data() as ParentApplication;
